 ...
 *
 * Callbacky jsou ulozeny globalne (jen 1 aktivni instance) pro maximalni vykon.
 * API je kompatibilni s cpu-z80 multi-v0.2 (drop-in replacement).
 *
 * Optimalizace:
 * - Globalni callback pointery (zadna dereference pres z80_t)
 * - Lokalni cache globalnich callbacku v z80_execute()
 * - __attribute__((optimize("O2"))) na z80_execute() - stabilni vykon i pri -O3
...
/* Vynutit O2 na execute smycce - O3 zpusobuje regresi u glob. fn pointers */
...
/* ========== Globalni callback context ========== */

/**
 * Globalni callbacky s user_data - signatura shodna s cpu-z80 multi-v0.2.
 * Ulozeny staticky pro rychly pristup bez dereference pres z80_t.
 * Omezeni: pouze jedna aktivni CPU instance.
 */
static z80_mread_cb  g_mread_cb  = NULL;  /**< Callback pro cteni pameti */
static void         *g_mread_data = NULL;
static z80_mwrite_cb g_mwrite_cb = NULL;  /**< Callback pro zapis pameti */
static void         *g_mwrite_data = NULL;
static z80_pread_cb  g_pread_cb  = NULL;  /**< Callback pro cteni I/O */
static void         *g_pread_data = NULL;
static z80_pwrite_cb g_pwrite_cb = NULL;  /**< Callback pro zapis I/O */
static void         *g_pwrite_data = NULL;
static z80_intread_cb g_intread_cb = NULL; /**< Callback pro cteni vektoru */
static void          *g_intread_data = NULL;
static z80_intack_cb  g_intack_cb = NULL;  /**< Callback pro INTACK */
static void          *g_intack_data = NULL;
static z80_reti_cb    g_reti_cb   = NULL;  /**< Callback pro RETI */
static void          *g_reti_data = NULL;
static z80_ei_cb      g_ei_cb    = NULL;  /**< Callback pro EI */
static void          *g_ei_data  = NULL;
/** Post-step callback - per-line WAIT v MZ-700 rezimu */
static void (*g_post_step_cb)(z80_t *, void *) = NULL;
static void *g_post_step_data = NULL;

/**
 * Pointer na jedinou aktivni CPU instanci.
 * Pouzivan v default callbackach a pro predani do user callbacku.
 */
...
/*
 * Pristup k pameti pro pouziti MIMO execute smycku (handle_interrupts).
 * Uvnitr z80_execute se pouzivaji makra s lokalni cache.
 */

/** Cteni bajtu z pameti (slow path - pristupuje ke globalnim). */
static inline u8 rd_slow(u16 addr) {
    return g_mread_cb(current_cpu, addr, 0, g_mread_data);
}

/** Zapis bajtu do pameti (slow path). */
static inline void wr_slow(u16 addr, u8 data) {
    g_mwrite_cb(current_cpu, addr, data, g_mwrite_data);
}

/** Cteni 16bit (slow path). */
static inline u16 rd16_slow(u16 addr) {
    u8 lo = rd_slow(addr);
    u8 hi = rd_slow(addr + 1);
    return (u16)(lo | (hi << 8));
}

/** Zapis 16bit (slow path). */
static inline void wr16_slow(u16 addr, u16 val) {
    wr_slow(addr, (u8)(val & 0xFF));
    wr_slow(addr + 1, (u8)(val >> 8));
}
 ...
 */
FORCE_O2 HOT int z80_execute(z80_t *cpu, int target_cycles) {
    current_cpu = cpu;

    /*
     * Lokalni cache globalnich callback pointeru.
     * GCC drzi tyto const hodnoty v callee-saved registrech,
     * eliminuje opakovanou dereferenci globalnich promennych.
     */
    const z80_mread_cb  _mrd  = g_mread_cb;
    void * const        _mrd_d = g_mread_data;
    const z80_mwrite_cb _mwr  = g_mwrite_cb;
    void * const        _mwr_d = g_mwrite_data;
    const z80_pread_cb  _prd  = g_pread_cb;
    void * const        _prd_d = g_pread_data;
    const z80_pwrite_cb _pwr  = g_pwrite_cb;
    void * const        _pwr_d = g_pwrite_data;
...
#define SET_WZL(v) do { rWZ = (rWZ & 0xFF00) | (u8)(v); } while(0)
     ...
     * Pouzivaji _mrd/_mwr/_prd/_pwr misto globalniho g_mread_cb atd.
     */
...
check_interrupts:
    {
        ...
        if (g_post_step_cb) {
            WRITEBACK();
            cpu->wait_cycles = 0;
            g_post_step_cb(cpu, g_post_step_data);
    ...
    op_FB: cpu->iff1 = 1; cpu->iff2 = 1; cpu->ei_delay = true;
           if (g_ei_cb) { WRITEBACK(); g_ei_cb(cpu, g_ei_data); RELOAD(); }
           NEXT_SLOW(4);
    ...
    op_ED: {
        ...
        switch (edop) {
            ...
            case 0x4D: case 0x5D: case 0x6D: case 0x7D:
                ...
                if (g_reti_cb) g_reti_cb(cpu, g_reti_data);
                NEXT_SLOW(14);
...
static int handle_interrupts_internal(z80_t *cpu) {
    ...
    /* Maskovane preruseni - blokovano po EI */
        ...
        if (g_intack_cb) g_intack_cb(cpu, g_intack_data);

        /* Cteni vektoru pres intread callback (pokud existuje a neni nastaven primo) */
        if (g_intread_cb && cpu->int_vector == 0) {
            cpu->int_vector = g_intread_cb(cpu, g_intread_data);
        }
...
/**
 * @brief Synchronizace globalnich callbacku z z80_t struktury.
 *
 * Kopiruje callback pointery a user_data ze struktury do globalnich promennych.
 * Volano po z80_create a z80_set_* funkcich.
 *
 * @param cpu Ukazatel na CPU instanci.
 */
static void sync_callbacks(z80_t *cpu) {
    g_mread_cb    = cpu->mread_cb;    g_mread_data    = cpu->mread_data;
    g_mwrite_cb   = cpu->mwrite_cb;   g_mwrite_data   = cpu->mwrite_data;
    g_pread_cb    = cpu->pread_cb;    g_pread_data    = cpu->pread_data;
    g_pwrite_cb   = cpu->pwrite_cb;   g_pwrite_data   = cpu->pwrite_data;
    g_intread_cb  = cpu->intread_cb;  g_intread_data  = cpu->intread_data;
    g_intack_cb   = cpu->intack_cb;   g_intack_data   = cpu->intack_data;
    g_reti_cb     = cpu->reti_cb;     g_reti_data     = cpu->reti_data;
    g_ei_cb       = cpu->ei_cb;       g_ei_data       = cpu->ei_data;
    g_post_step_cb = cpu->post_step_cb; g_post_step_data = cpu->post_step_data;
    current_cpu = cpu;
}
...
z80_t *z80_create(
    ...
    /* Nastaveni callbacku do struktury */
    ...
    /* Synchronizace do globalnich promennych */
    sync_callbacks(cpu);
...
void z80_destroy(z80_t *cpu) {
    if (cpu == current_cpu) current_cpu = NULL;
    free(cpu);
}
